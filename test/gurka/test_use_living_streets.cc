...
protected:
  ...
  static void SetUpTestSuite() {
    ...
    use_living_streets_map = gurka::buildtiles(layout, ways, {}, {}, "test/data/gurka_use_living_streets");
...
TEST_F(LivingStreetTest, test_default_value) {
  for (const auto& c : costing)
    if (c == "bicycle" || c == "pedestrian")
      ...
      validate_path(gurka::route(use_living_streets_map, "1", "2", c), {"AB", "BE", "EF"});
    else
      // avoid living_streets by default; use living_streets only if the route starts or ends at 'living_street' edge
      validate_path(gurka::route(use_living_streets_map, "1", "2", c), {"AB", "BC", "CD", "DE", "EF"});
}
...
TEST_F(LivingStreetTest, test_use_living_streets_if_no_other_roads) {
  ...
  for (const auto& c : costing) {
    validate_path(gurka::route(use_living_streets_map, "D", "3", c,
                               {{"/costing_options/" + c + "/use_living_streets", "0"}}),
                  {"DE", "EF", "FG"});
  }
}
